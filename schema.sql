...
create table sessions(ssid int(3),tname char(15),pname char(15),pst_date date,primary key (ssid));
	
create table idgeneration(ssid int(3),timestamp int(60),foreign key (ssid) references sessions(ssid),primary key (timestamp));

create table feedback(ssid int(3),user char(50),pskills int(5),tskills int(5),timestamp int(60),status b
ool,foreign key (timestamp) references idgeneration(timestamp),foreign key (ssid) references sessions(ssid),primary key(timestamp));
